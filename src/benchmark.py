...
class BenchmarkRunner:
    ...
    def run(self, config_path: str, overrides: Optional[dict] = None) -> BenchmarkMetrics:
        ...
        gates = QualityGates(
            ...
            native_rg=native_rg,
            enable_shape_gate=bool(cfg.get("quality.enable_shape_gate", False)))
        ...
        extractor = PoseExtractor(
            "largest_cluster" if cfg.get("docking.pose_selection", "lowest_energy")
            == "largest_cluster" else "lowest_energy")
        base_seed = int(cfg.get("docking.seed", seed))
        n_reps = max(1, int(cfg.get("docking.consensus_runs", 1)))
